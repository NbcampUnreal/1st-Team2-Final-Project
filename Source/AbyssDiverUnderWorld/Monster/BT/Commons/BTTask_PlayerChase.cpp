#include "Monster/BT/Commons/BTTask_PlayerChase.h"
...
EBTNodeResult::Type UBTTask_PlayerChase::ExecuteTask(UBehaviorTreeComponent& Comp, uint8* NodeMemory)
{
	...
	TaskMemory->Monster = Cast<AMonster>(TaskMemory->AIController->GetCharacter());
	...
	TaskMemory->Monster->SetMonsterState(EMonsterState::Chase);

	// 최대로 플레이어를 쫓을 시간을 초기화하는 코드이다.
	// ChaseAccumulatedTime이 FinishTaskInterval을 넘어가면 추적을 중단한다.
	TaskMemory->Monster->ChaseAccumulatedTime = 0.f;
	...
	TaskMemory->Monster->SetMaxSwimSpeed(ChaseMoveSpeed);
...
void UBTTask_PlayerChase::TickTask(UBehaviorTreeComponent& Comp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	AUnderwaterCharacter* Player = Cast<AUnderwaterCharacter>(TaskMemory->Monster->GetTarget());
	if (IsValid(Player))
	{
		if (Player->IsDeath() || Player->IsGroggy())
		{
			AEnhancedBossAIController* EnhancedAIController = Cast<AEnhancedBossAIController>(TaskMemory->AIController);
			if (EnhancedAIController)
			{
				EnhancedAIController->InitVariables();
			}
	...
	// 플레이어가 시야에서 사라진 경우
	AEnhancedBossAIController* EnhancedAIController = Cast<AEnhancedBossAIController>(TaskMemory->AIController);
	if (EnhancedAIController&& EnhancedAIController->GetIsDisappearPlayer())
	{
		if (IsValid(Player))
		{
	...
	if (TaskMemory->Monster->ChaseAccumulatedTime > TaskMemory->FinishTaskInterval)
	{
		TaskMemory->AIController->SetBlackboardPerceptionType(EPerceptionType::Finish);
		LOGV(Log, TEXT("Chase Finished, Time Elapsed: %f, Time Limit: %f"), TaskMemory->Monster->ChaseAccumulatedTime, TaskMemory->FinishTaskInterval);
	}

	TaskMemory->Monster->ChaseAccumulatedTime += DeltaSeconds;
	LOGV(Log, TEXT("Chase Accumulated Time: %f"), TaskMemory->Monster->ChaseAccumulatedTime);
	...
	EBTNodeResult::Type TaskResult)
{
	...
	if (!TaskMemory->Monster.IsValid() || !TaskMemory->AIController.IsValid()) return;

	// 추적 상태가 끝나는 경우 감속과 이동속도를 원래 상태로 복원한다.
	TaskMemory->Monster->InitCharacterMovementSetting();
}

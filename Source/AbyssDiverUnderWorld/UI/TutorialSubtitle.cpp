...
{
	if (TargetTextIndex >= TargetText.Len() && CurrentTypingStep == 0)
	{
		GetWorld()->GetTimerManager().ClearTimer(TypingTimerHandle);
	...
	while (TargetTextIndex < TargetText.Len())
	{
		if (TargetText[TargetTextIndex] == '<')
		{
			int32 EndTagIndex = TargetText.Find(TEXT(">"), ESearchCase::IgnoreCase, ESearchDir::FromStart, TargetTextIndex);
			if (EndTagIndex != INDEX_NONE)
			{
				CurrentDisplayText += TargetText.Mid(TargetTextIndex, EndTagIndex - TargetTextIndex + 1);
				TargetTextIndex = EndTagIndex + 1;
				continue;
			}
		}
		...
		if (FHangul::Decompose(CurrentChar, Cho, Jung, Jong))
		{
			...
			{
				if (!CurrentDisplayText.IsEmpty() && FChar::IsAlpha(CurrentDisplayText[CurrentDisplayText.Len() - 1]))
				{

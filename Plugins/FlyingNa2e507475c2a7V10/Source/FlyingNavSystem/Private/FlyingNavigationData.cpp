...
#if ATLEAST_UE5_5
		RaycastImplementationWithAdditionalResults = OctreeRaycastWithAdditionalResults;
#else
...
#if UE5
	TObjectPtr<UPathBatchComponent> PathBatcher = This->PathBatcher;
#else
	UPathBatchComponent* PathBatcher = This->PathBatcher;
#endif
	
	Collector.AddReferencedObject(PathBatcher, This);
#endif // WITH_EDITOR
...
void AFlyingNavigationData::BeginPlay()
{
	...
	LevelInstanceOffset = GetLevelTransform().GetLocation();
	if (bApplyLevelTransformOnBeginPlay)
	{
		SVOData->ApplyWorldOffset(LevelInstanceOffset);
	}
...
void AFlyingNavigationData::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	// Ensure dynamic building is finished or cancelled.
...
void AFlyingNavigationData::OnOctreeGenerationFinished()
{
	...
	{
		// Swaps building navdata into current navdata.
		UpdateCurrentNavData();
...
FPathFindingResult AFlyingNavigationData::FindPath(const FNavAgentProperties& AgentProperties, const FPathFindingQuery& Query)
{
	...
	const TWeakObjectPtr<const AFlyingNavigationData> WeakFlyingNavData = FlyingNavData;
	ON_SCOPE_EXIT
	{
		if (bInAsyncThread)
		{
			if (const AFlyingNavigationData* NavData = WeakFlyingNavData.Get())
			{
				// Manually unlock critical section
				NavData->AsyncGraphCriticalSection.Unlock();

				// Make sure number of paths is kept up to date
				NavData->DecrementCalculatingPaths();
			}
		}
	};
	...
	// Make sure we have a navigation graph. This could happen on EndPlay if the navdata is destroyed before async task competes.
...
bool AFlyingNavigationData::OctreeRaycast(const ANavigationData* Self,
...
#if ATLEAST_UE5_5
bool AFlyingNavigationData::OctreeRaycastWithAdditionalResults(const ANavigationData* Self,
                                                               const FVector& RayStart,
                                                               const FVector& RayEnd,
                                                               FVector& HitLocation,
                                                               ...
                                                               const UObject* Querier)
{
	...
	{
		// Corridors are not an octree concept.
		AdditionalResults->bIsRayEndInCorridor = false;
	}
	return OctreeRaycast(Self, RayStart, RayEnd, HitLocation, QueryFilter, Querier);
}
#endif // ATLEAST_UE5_5

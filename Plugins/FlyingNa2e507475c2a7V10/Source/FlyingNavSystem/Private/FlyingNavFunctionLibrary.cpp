...
{
	struct FDelegateHelper final
	{
		TWeakObjectPtr<AActor> GoalActor;
		TWeakObjectPtr<UNavigationPath> WeakPath;
		...
		{
			if (UNavigationPath* Path = WeakPath.Get())
			{
				Path->SetPath(NavPath);
				if (Path->GetPath())
				{
					if (FFlyingNavigationPath* FlyingPath = Path->GetPath()->CastPath<FFlyingNavigationPath>())
					{
						FlyingPath->QueryResult = Result;
					}

					if ((Result == ENavigationQueryResult::Success ||
						(NavPath && NavPath->IsPartial())) && GoalActor.IsValid())
					{
...
public:
	FPathFindAction(UNavigationPath* NavPath,
		...
		: ExecutionFunction(LatentInfo.ExecutionFunction),
		  OutputLink(LatentInfo.Linkage),
		  CallbackTarget(LatentInfo.CallbackTarget),
		  Delegate(MakeShared<FDelegateHelper>(NavPath, GoalActor, TetherDistance))
	{
	...
	{
		// Cleanup async query if not already finished.
		if (!Delegate.Get().bFinished && AsyncQueryHandle.IsValid())
		{
			if (UNavigationPath* Path = Delegate->WeakPath.Get())
			{
				if (UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Path))
				{

...
void FRasteriseWorker::DoWork()
{
	...
	// The worker should always be able to call OnRasteriserWorkerFinished, because the parent generator should always be valid.
	// Currently, on cancel some worker threads outlive the parent generator, and this is a workaround.
#if ALLOW_CANCEL
    if (ShouldAbort())
    {
        return;
...
#if ATLEAST_UE5_5
			TStrongObjectPtr<const UObject> OwnerGuard = Element.GetSourceElement()->GetWeakUObject().Pin();
			 const UObject* Owner = OwnerGuard.Get();
#else
			 const UObject* Owner = Element.GetOwner();
#endif // ATLEAST_UE5_5

			bool bShouldUse = Owner != nullptr && Element.ShouldUseGeometry(OwnerNavDataConfig);

			// Don't bother with simulated objects
			if (const UStaticMeshComponent* StaticMeshComp = Cast<const UStaticMeshComponent>(Owner))
			{
...
{
#if ATLEAST_UE5_5
	const TStrongObjectPtr<UNavigationSystemV1> NavSysPin = NavSys.Pin();
#endif // ATLEAST_UE5_5
	...
	for (TSharedRef<FNavigationRelevantData, ESPMode::ThreadSafe>& ElementData : NavigationRelevantData)
	{
		...
		ElementData->SourceElement->GeometrySliceExportDelegate.ExecuteIfBound(ElementData->SourceElement.Get(), GeomExport, GenerationBoundsExpandedForAgent);
#else
			if (INavRelevantInterface* NavRelevant = Cast<INavRelevantInterface>(ElementData->GetOwner()))
			{
...
void FRasteriseWorker::ValidateAndAppendGeometry(const FNavigationRelevantData& ElementData)
{
	...
	{
#if ATLEAST_UE5_5
		UE_LOG(LogFlyingNavSystem, Error, TEXT("NavOctree element has corrupted collision data! Element:%s Bounds:%s"), *ElementData.SourceElement->GetName(), *ElementData.Bounds.ToString());
#else
...
void FFlyingNavigationDataGenerator::TickAsyncBuild(float DeltaSeconds)
{
	...
	if (GeneratorTask.IsValid())
	{
		...
		{
			// Little bit of a hack here: we pass 0 to DeltaSeconds in EnsureBuildCompletion to force the task to finish.
			const bool bBlockThread = DeltaSeconds == 0.f;
			GeneratorTask->RasteriseTick(bBlockThread);
		}
	}
}

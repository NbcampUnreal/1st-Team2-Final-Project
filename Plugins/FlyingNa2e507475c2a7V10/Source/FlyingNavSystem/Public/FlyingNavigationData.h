...
public:
	...
	// Whether the transform of the level instance should be applied to the navigation data on begin play.
	UPROPERTY(EditAnywhere, Category = Generation, Config, AdvancedDisplay)
	uint32 bApplyLevelTransformOnBeginPlay: 1;
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = Pathfinding, Config, meta=(ClampMin=1, UIMin=1, ClampMax=5, UIMax=5))
...
#if ATLEAST_UE5_5
	static bool OctreeRaycastWithAdditionalResults(const ANavigationData* Self, const FVector& RayStart, const FVector& RayEnd, FVector& HitLocation, FNavigationRaycastAdditionalResults* AdditionalResults, FSharedConstNavQueryFilter QueryFilter, const UObject* Querier);
#endif // ATLEAST_UE5_5
...
protected:
	...
	FVector LevelInstanceOffset = FVector::ZeroVector;
	...
	UPROPERTY(Transient)
	class UPathBatchComponent* PathBatcher;
